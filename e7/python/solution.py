...
class Solution:
    ...
    def serialize(self, root):
        ...
        if not root:
            return result
        parent = [root]
        result.append(self.getNodeStr(root))
        while not self.isAllNone(parent):
            new_parent = []
            for node in parent:
                if node:
                    result.append(self.getNodeStr(node.left))
                    result.append(self.getNodeStr(node.right))
                    new_parent.append(node.left)
                    new_parent.append(node.right)
            parent = new_parent
        return ','.join(result)
